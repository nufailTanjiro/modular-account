...
import {UUPSUpgradeable} from "@alchemy/light-account/lib/modular-account/ext/UUPSUpgradeable.sol";
import {PluginManifest, PluginMetadata, ManifestFunction, ManifestAssociatedFunction, ManifestAssociatedFunctionType, SelectorPermission} from "@alchemy/light-account/lib/modular-account/src/interfaces/IPlugin.sol";
import {BasePlugin} from "@alchemy/light-account/lib/modular-account/src/plugins/BasePlugin.sol";
...
import {UserOperation} from "@alchemy/light-account/lib/modular-account/src/interfaces/erc4337/UserOperation.sol";
import {SIG_VALIDATION_FAILED, SIG_VALIDATION_PASSED} from "@alchemy/light-account/lib/modular-account/src/libraries/Constants.sol";
import {CastLib} from "@alchemy/light-account/lib/modular-account/src/helpers/CastLib.sol";
import {IStandardExecutor} from "@alchemy/light-account/lib/modular-account/src/interfaces/IStandardExecutor.sol";
// import {UpgradeableModularAccount} from "@alchemy/light-account/lib/modular-account/src/account/UpgradeableModularAccount.sol";
...
contract MultisigPlugin is BasePlugin, IMultisigPlugin, IERC1271 {
    ...
    function onUninstall(bytes calldata) external override {
        address[] memory ownersToRemove = CastLib.toAddressArray(
            _owners.getAll(msg.sender)
        );
    ...
    function userOpValidationFunction(
        ...
        if (functionId == uint8(FunctionId.USER_OP_VALIDATION_OWNER)) {
            ...
            bytes32 upperLimitDigest = (upperLimitPreVerificationGas ==
    ...
    function supportsInterface(
        ...
        return
            interfaceId == type(IMultisigPlugin).interfaceId ||
    ...
    function checkNSignatures(
        ...
        uint256 numSigsOnActualGas = (actualDigest != upperLimitGasDigest)
            ? 1
            : 0;
        success = true;
        ...
        for (uint256 i = 0; i < threshold; i++) {
            ...
            if (v == 0) {
                ...
                bytes memory contractSignature;
                {
                    ...
                    assembly ("memory-safe") {
                        ...
                        totalContractSigLen := add(
                            mload(contractSignature),
                            0x20
                        ) // prefixed 32 bytes of len(bytes)
                    }
            ...
            if (
                ...
                !_owners.contains(account, CastLib.toSetValue(currentOwner))
            ) {
                if (success) {
                    firstFailure = i;
                    success = false;
    ...
    function encodeMessageData(
        ...
        bytes32 messageHash = keccak256(
            abi.encode(_MULTISIG_PLUGIN_TYPEHASH, keccak256(message))
        );
        return
            abi.encodePacked(
                "\x19\x01",
    ...
    function _onInstall(
        ...
        (address[] memory initialOwners, uint128 threshold) = abi.decode(
            data,
            (address[], uint128)
        );
